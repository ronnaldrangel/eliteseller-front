...
export default async function AccountPage({ params }) {
  ...
  const chatbotParam = await params
  const chatbotSegment = String(chatbotParam?.chatbot || "")

  if (chatbotSegment) {
    const owned = await getChatbotBySlug(
      chatbotSegment,
      session.strapiToken,
  ...
  try {
    ...
    if (!res.ok) {
      ...
      error =
        ...
        `No se pudo obtener la informacion de la cuenta (status ${res.status})`
    } else {
      me = await res.json()
    }
  } catch (_) {
