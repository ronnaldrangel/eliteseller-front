      ...
      async authorize(credentials) {
        ...
        try {
          ...
          if (response.ok && data.user) {
            ...
            // Obtener el rol del usuario desde Strapi
            let userRole = null;
            try {
              const meUrl = buildStrapiUrl("/api/users/me?populate=role");
          ...
          if (response.status === 400) {
            ...
            // También verificar el mensaje anterior por si acaso
              ...
              const userCheckResponse = await strapiUsers.findByEmail(
                credentials.email
              );
              ...
              if (userCheckResponse.ok) {
                ...
                if (userData.length > 0) {
                  ...
                  if (!user.confirmed) {
                    ...
                    console.log(
                      "🚫 Preventing authentication for unconfirmed user"
                    );
                    // NO autenticar al usuario, solo retornar null
                    return null;
                  }
    ...
    async signIn({ account, profile }) {
      ...
      if (account?.provider === "google") {
        try {
          ...
          const callbackUrl = buildStrapiUrl(
            `/api/auth/google/callback?access_token=${account.access_token}`
          );
          ...
          if (!authRes.ok) {
            console.error(
              `❌ Error en callback OAuth de Strapi: ${authRes.status}`
            );
            return false;
          }
          ...
          console.log(
            `✅ Usuario OAuth sincronizado con Strapi: ${authData.user.email}`
          );
          return true;
        } catch (error) {
    ...
    async jwt({ token, user, account }) {
      ...
      if (account?.provider === "google") {
        try {
          ...
          if (!authRes.ok) {
            console.error(
              `❌ Error en callback OAuth de Strapi (jwt): ${authRes.status}`
            );
            return token;
          }
          ...
          // Obtener el rol del usuario desde Strapi (Google Auth)
          let googleUserRole = null;
          try {
            const meUrl = buildStrapiUrl("/api/users/me?populate=role");
            const meRes = await fetch(meUrl, {
            ...
            if (meRes.ok) {
              const meData = await meRes.json();
              googleUserRole = meData.role?.name;
              console.log("👤 User role (Google):", googleUserRole);
            }
          } catch (roleError) {
            console.error("❌ Error fetching Google user role:", roleError);
          }
          ...
          token.name =
            authData.user.name || authData.user.username || authData.user.email;
          token.emailVerified = true;
          token.role = googleUserRole;
          console.log(
            "🔐 jwt(Google): token.strapiToken (full)",
            token.strapiToken
          );
      ...
      if (user?.strapiToken) {
        ...
        console.log(
          "🔐 jwt(Credentials): token.strapiToken (full)",
          token.strapiToken
        );
        console.log(
          "🔐 jwt(Credentials): token.strapiUserId",
          token.strapiUserId
        );
      }
      return token;
        ...
        if (typeof data.error === "string") {
          ...
          if (details.errors && details.errors.length > 0) {
            ...
            if (firstError.path && firstError.path.includes("email")) {
              ...
              firstError.path &&
              firstError.path.includes("username")
            ) {
